...
void ABlurAbilityWeapon::HandleEquipWeapon(AActor* SelfOwner)
{
	if (const ABlurCharacterBase* BlurCharacterBase = Cast<ABlurCharacterBase>(SelfOwner))
	{
		// 赋予拥有者武器技能。
...
void ABlurAbilityWeapon::HandleUnequipWeapon(AActor* SelfOwner)
{
	if (const ABlurCharacterBase* BlurCharacterBase = Cast<ABlurCharacterBase>(SelfOwner))
	{
		// 清除赋予拥有者的技能。
